...
const ProjectForm: React.FC<ProjectFormProps> = ({
  ...
  const [formData, setFormData] = useState<ProjectFormData>({
    ...
    phases: [
      { name: 'Studies', teamLeaderId: '' },
      { name: 'Design', teamLeaderId: '' },
      { name: 'Technical', teamLeaderId: '' },
    ],
  });
  ...
  useEffect(() => {
    const loadTeamLeaders = async () => {
      try {
        const response = await apiClient.get<{ users: any[] }>('/users?role=TEAM_LEADER');
        setTeamLeaders(response.users || []);
      } catch (error) {
                ...
                <Grid container spacing={2}>
                  <Grid xs={12} sm={6}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        value={phase.teamLeaderId}
                        onChange={e => {
                          const updatedPhases = [...(formData.phases || [])];
                          updatedPhases[index].teamLeaderId = e.target.value as string;
