...
router.get('/', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const filter = {
      ...
      currentPhase: currentPhase as unknown as string,
...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      logger.error('Failed to get project dashboard', {
        error,
...
router.get('/:id/phases', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ error: 'Unauthorized' });
      return;
...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      logger.error('Failed to get project requirements', {
        error,
        projectId: req.params.id as string,
      });
...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      logger.error('Failed to get project modifications', {
        error,
        projectId: req.params.id as string,
      });
