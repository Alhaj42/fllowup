...
class TimelineService {
    ...
    async getTimeline(filters) {
        try {
            ...
            const timelineData = await Promise.all(projects.map(async (project) => {
                ...
                const teamAssignmentMap = new Map();
            ...
            logger_1.default.info('Timeline data retrieved successfully', { projectCount: timelineData.length, conflictCount: conflicts.length });
            return timelineData;
        }
        catch (error) {
            logger_1.default.error('Failed to retrieve timeline data', { error, filters });
            throw error;
        }
    }
    /**
     ...
     */
    detectConflicts(timelineData, startDate, endDate) {
        const conflicts = [];
        // Detect phase overlaps within the same project
        const projectPhaseRanges = new Map();
        ...
        const teamMemberAllocations = new Map();
        timelineData.forEach(project => {
    ...
    async getCalendarEvents(year, month) {
        try {
            ...
            logger_1.default.info('Calendar events retrieved successfully', { year, month, eventCount: events.length });
            return events;
        }
        catch (error) {
            logger_1.default.error('Failed to retrieve calendar events', { error, year, month });
            throw error;
        }
    }
    /**
     ...
     */
    async logTimelineAccess(userId, action, details) {
        try {
            await auditLogService_1.default.logCreate('TimelineAccess', `access-${Date.now()}`, userId, 'MANAGER', // Assuming managers can access timeline
            {
                action,
                details,
                timestamp: new Date()
            });
            logger_1.default.info('Timeline access logged', { userId, action });
        }
        catch (error) {
            logger_1.default.error('Failed to log timeline access', { error, userId });
        }
    }
}
exports.default = new TimelineService();
